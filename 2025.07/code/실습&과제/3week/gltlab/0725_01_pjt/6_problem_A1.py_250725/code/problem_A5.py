...
if file_path.exists():  # 파일이 존재할 경우
    ...
    for item in data['item']:  # 'item' 리스트의 각 항목을 순회
        ...
        if price < 10000:
            price_categories['1만원미만'].append(title)
        elif price >= 20000:
            price_categories['2만원이상'].append(title)
        else:
            price_categories['1만원이상 2만원 미만'].append(title)
        # price_categories[item["priceStandard"]] = item["title"]
