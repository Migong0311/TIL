...
def get_postfix(infix, N):
    ...
    # 중위표기식에서 글자(토큰) 하나씩 때어와서 후위표기식을 만들자
    for token in infix:
        # 토큰이 피연산자인경우
        if token not in '()+-*/':
            # 후위표기식에 그대로 피연산자를 쓴다(출력)
            postfix += token
            # 토큰이 연산자인경우
        else:
            # 토큰이 닫는괄호라면
            if token == ')':
                # '(...)' 안에 있는 모든 연산자가 먼저 계산되어야함
                # 스택안에 여는괄호를 만날떄까지 모든 연산자를 꺼내 쓴다
                while stack:
                    # 연산자를 하나 꺼내보기
                    operator = stack.pop()
                    # 꺼낸게 여는괄호라면
                    ...
                    # 여는괄호가 아니라면 계속 식에 출력
                        ...
                        postfix += operator
            # 토큰이 다든ㄴ 괄확 아닌 다른 연산자였다면
            else:
                # 스택 꼭대기에 있는 연산자의 우선순위와 -> isp[stack[-1]]
                # 현재 토큰의 우선순위를 비교 -> icp[token]
                # 누가 더 우선순위가 높은지 확인
                # 우선순위가 높은 연산자는 먼저 계산이 되어야 하니 출력

                # 스택 안에 현재 token 보다 우선순위가 같거나 높은 연산자는 다 꺼내야한다

                while stack and icp[token] <= isp[stack[-1]]:
                    # 꺼내서 결과에 이어붙이기
                    postfix += stack.pop()

                    # token의 우선순위가 스택의 꼭대기에 있는 연산자의 우선순위보다 높은다면
                    # 또는 스택이 비어있다면
                stack.append(token)

    # 모든 토큰을 확인한후에 스택에 남아있는 연산자는 그대로 차례대로 출력
    while stack:
        postfix += stack.pop()
    return postfix
